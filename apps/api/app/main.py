...
def ensure_demo_control_plane():
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur:
            ...
            if not tenant_row: raise RuntimeError("cyberos-demo tenant is not initialized")
...
@app.get("/")
def root(): return {"product":"CyberOS","service":"control-plane","milestone":"M3","status":"online"}
...
def health():
    ...
    return {"status":"healthy" if database=="healthy" else "degraded","database":database,"timestamp":datetime.now(timezone.utc).isoformat()}

@app.get("/api/v1/platform")
def platform():
    return {"name":"CyberOS","version":"0.3.1-m3","milestone":"M3 Customer Security Workspace + Agent Gateway + Assessment Reports","modules":[name for _,name in MODULES],"execution":"policy-controlled","api_port":8000,"portal_port":3100,"network_execution":"customer-agent-only; authorization required"}
...
def module_catalog():
    ...
    return [{"key":k,"name":n,**enabled.get(k,{"enabled":False,"configuration":{}})} for k,n in MODULES]
...
def set_module(module_key:str,enabled:bool=Query(...)):
    ...
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("INSERT INTO audit_events (tenant_id,actor_id,event_type,resource_type,decision,metadata) VALUES (%s,%s,'tenant.module.changed','tenant_module','allow',%s)",(tenant_id,actor_id,json.dumps({"module":module_key,"enabled":enabled})))
            conn.commit()
...
def register_connector(req:ConnectorRequest):
    ...
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("INSERT INTO audit_events (tenant_id,actor_id,event_type,resource_type,resource_id,decision,metadata) VALUES (%s,%s,'connector.registered','connector',%s,%s,%s)",(tenant_id,actor_id,connector_id,"allow" if allowed else "review",json.dumps({"name":req.name,"capabilities":req.capabilities})))
            conn.commit()
...
def audit(limit:int=Query(default=25,ge=1,le=100)):
    tenant_id,_,_=ensure_demo_control_plane()
    rows=db_fetch("SELECT ae.id,ae.event_type,ae.resource_type,ae.resource_id,ae.decision,ae.metadata,ae.created_at,i.display_name FROM audit_events ae LEFT JOIN identities i ON i.id=ae.actor_id AND i.tenant_id=ae.tenant_id WHERE ae.tenant_id=%s ORDER BY ae.created_at DESC LIMIT %s",(tenant_id,limit))
...
def current_tenant():
    tenant_id,_,_=ensure_demo_control_plane(); row=db_fetch("SELECT id,name,slug,industry,region,subscription_tier,status,created_at FROM tenants WHERE id=%s",(tenant_id,))[0]
...
def current_identity():
    _,actor_id,_=ensure_demo_control_plane(); row=db_fetch("SELECT id,email,display_name,role,status,created_at FROM identities WHERE id=%s",(actor_id,))[0]
...
def demo_summary():
    tenant=db_fetch("SELECT id,name,subscription_tier FROM tenants WHERE slug='cyberos-demo'")[0]; tenant_id=tenant[0]
    assets_count=db_fetch("SELECT COUNT(*) FROM assets WHERE tenant_id=%s",(tenant_id,))[0][0]; findings=db_fetch("SELECT COUNT(*) FROM findings WHERE tenant_id=%s AND status='open'",(tenant_id,))[0][0]; connectors=db_fetch("SELECT COUNT(*) FROM connectors WHERE tenant_id=%s AND status='online'",(tenant_id,))[0][0]; frameworks=db_fetch("SELECT COUNT(*) FROM compliance_frameworks")[0][0]; jobs=db_fetch("SELECT COUNT(*) FROM jobs WHERE tenant_id=%s",(tenant_id,))[0][0]
    return {"tenant":{"id":str(tenant[0]),"name":tenant[1],"tier":tenant[2]},"assets":assets_count,"open_findings":findings,"online_connectors":connectors,"frameworks":frameworks,"jobs":jobs,"execution_policy":"enforced"}

@app.get("/api/v1/demo/assets")
def demo_assets():
    tenant_id=db_fetch("SELECT id FROM tenants WHERE slug='cyberos-demo'")[0][0]; rows=db_fetch("SELECT id,name,asset_type,identifier,environment,criticality,exposure FROM assets WHERE tenant_id=%s ORDER BY name",(tenant_id,))
...
def demo_findings():
    tenant_id=db_fetch("SELECT id FROM tenants WHERE slug='cyberos-demo'")[0][0]; rows=db_fetch("SELECT id,title,severity,category,status,remediation FROM findings WHERE tenant_id=%s ORDER BY CASE severity WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END,title",(tenant_id,))
...
def demo_jobs():
    tenant_id=db_fetch("SELECT id FROM tenants WHERE slug='cyberos-demo'")[0][0]; rows=db_fetch("SELECT id,capability,target,state,policy_reason,created_at,updated_at FROM jobs WHERE tenant_id=%s ORDER BY created_at DESC LIMIT 25",(tenant_id,))
...
def request_assessment(job:JobRequest):
    ...
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO jobs (tenant_id,requested_by,authorization_id,capability,target,state,policy_reason,parameters) VALUES (%s,%s,%s,%s,%s,'queued',%s,%s) RETURNING id,created_at,updated_at",(tenant_id,actor_id,auth_id,job.capability,job.target,"customer-agent execution",json.dumps({"connector_id":job.connector_id,**job.parameters})))
            job_id,created_at,updated_at=cur.fetchone(); conn.commit()
    return {"id":str(job_id),"state":"queued","mode":"customer-agent","execution":"customer agent gateway","created_at":created_at.isoformat(),"updated_at":updated_at.isoformat()}
