...
class JobRequest(BaseModel):
    capability:str=Field(min_length=1,max_length=100); target:str=Field(min_length=1,max_length=500); authorized:bool=False; connector_id:str|None=None; parameters:dict=Field(default_factory=dict)
class ConnectorRequest(BaseModel):
    name:str=Field(min_length=2,max_length=120); connector_type:str=Field(default="customer_agent",max_length=50); environment:str=Field(default="hybrid",max_length=40); endpoint:str|None=Field(default=None,max_length=500); capabilities:list[str]=Field(default_factory=list)
def db_fetch(query,params=()):
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur: cur.execute(query,params); return cur.fetchall()
def ensure_demo_control_plane():
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id FROM tenants WHERE slug='cyberos-demo'"); tenant_row=cur.fetchone()
            if not tenant_row: raise RuntimeError("cyberos-demo tenant is not initialized")
            tenant_id=tenant_row[0]
            cur.execute("INSERT INTO identities (tenant_id,email,display_name,role) VALUES (%s,'operator@demo.cyberos.local','Demo Security Operator','security_operator') ON CONFLICT (tenant_id,email) DO UPDATE SET role=EXCLUDED.role RETURNING id",(tenant_id,)); actor_id=cur.fetchone()[0]
            cur.execute("SELECT id FROM authorization_contexts WHERE tenant_id=%s AND engagement_name='CyberOS Synthetic Demo Authorization' ORDER BY created_at DESC LIMIT 1",(tenant_id,)); existing=cur.fetchone()
            if existing: auth_id=existing[0]
            else:
                cur.execute("INSERT INTO authorization_contexts (tenant_id,engagement_name,authorized_by,scope,exclusions,allowed_capabilities,starts_at,ends_at,status) VALUES (%s,'CyberOS Synthetic Demo Authorization',%s,%s,%s,%s,now(),now()+interval '365 days','active') RETURNING id",(tenant_id,actor_id,json.dumps(["https://portal.demo.cyberos.local","api.demo.cyberos.local","demo-linux-01","demo-edge-fw"]),json.dumps([]),json.dumps(sorted(SAFE_CAPABILITIES))); auth_id=cur.fetchone()[0]
            conn.commit(); return str(tenant_id),str(actor_id),str(auth_id)
...
def organization_workspace():
    tenant_id,actor_id,auth_id=ensure_demo_control_plane(); tenant=db_fetch("SELECT id,name,slug,industry,region,subscription_tier,status FROM tenants WHERE id=%s",(tenant_id,))[0]; modules=db_fetch("SELECT module_key,enabled,configuration FROM tenant_modules WHERE tenant_id=%s ORDER BY module_key",(tenant_id,)); connectors=db_fetch("SELECT id,name,status,connector_type,environment,endpoint,last_seen,capabilities,authorization_id FROM connectors WHERE tenant_id=%s ORDER BY name",(tenant_id,)); assets=db_fetch("SELECT id,name,asset_type,identifier,environment,criticality,exposure FROM assets WHERE tenant_id=%s ORDER BY name",(tenant_id,))
...
def set_module(module_key:str,enabled:bool=Query(...)):
    ...
    with psycopg.connect(DB,connect_timeout=3) as conn:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO tenant_modules (tenant_id,module_key,enabled,updated_at) VALUES (%s,%s,%s,now()) ON CONFLICT (tenant_id,module_key) DO UPDATE SET enabled=EXCLUDED.enabled,updated_at=now()",(tenant_id,module_key,enabled)); cur.execute("INSERT INTO audit_events (tenant_id,actor_id,event_type,resource_type,decision,metadata) VALUES (%s,%s,'tenant.module.changed','tenant_module','allow',%s)",(tenant_id,actor_id,json.dumps({"module":module_key,"enabled":enabled}))); conn.commit()
