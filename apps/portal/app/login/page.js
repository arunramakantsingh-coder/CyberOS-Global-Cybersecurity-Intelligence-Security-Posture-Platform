...
export default function LoginPage() {
  ...
  const [message, setMessage] = useState('Demo authentication only — no customer credentials are stored.');

  const enterDemo = (event) => {
    event.preventDefault();
    setMessage('Authenticated demo operator → tenant context loaded → policy boundary active.');
  };
  ...
  return (
    <main className="login-shell">
      <section className="login-card">
        ...
        <p>Enterprise users enter through a tenant-aware identity boundary. After authentication, the portal loads organization context, entitlements, policy and authorized security capabilities.</p>
        <form onSubmit={enterDemo}>
          <label>Work email<input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required /></label>
          <label>Password<input type="password" placeholder="Demo mode" required /></label>
