...
const demoHref = '/login?mode=register';

export default function PublicWebsite() {
  return (
    <div className="site-shell">
      <header className="site-nav">
        <Link href="/" className="site-brand"><span className="site-mark">C</span><span className="site-brand-text"><b>CYBEROS</b><small>GLOBAL SECURITY PLATFORM</small></span></Link>
        ...
        <div className="site-actions"><Link href="/login" className="site-secondary">Command Center</Link><Link href={demoHref} className="site-primary">Explore Demo →</Link></div>
      </header>
      ...
      <main>
        <section className="hero" id="platform">
          <div className="hero-copy">
            ...
            <div className="hero-actions"><Link href={demoHref} className="site-primary large">Enter Demo World →</Link><a href="#modules" className="site-secondary large">Explore the platform</a></div>
        ...
        <section className="trust-strip"><span>Designed for modern security teams</span><b>FINANCIAL SERVICES</b><b>GOVERNMENT</b><b>MANUFACTURING</b><b>HEALTHCARE</b><b>TECHNOLOGY</b></section>

        <section className="section" id="modules"><div className="section-heading"><span className="site-eyebrow">THE CYBEROS PLATFORM</span><h2>Security capabilities that work together.</h2><p>CyberOS is designed as one operating model rather than a collection of disconnected security products.</p></div><div className="public-module-grid">{modules.map(([title, desc], i) => <article className="public-card" key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{desc}</p><Link href={demoHref}>Explore capability →</Link></article>)}</div></section>

        <section className="split-section" id="compliance"><div><span className="site-eyebrow">EVIDENCE-DRIVEN ASSURANCE</span><h2>Turn technical security work into compliance evidence.</h2><p>Map findings, assessments and evidence to versioned controls so one technical observation can support multiple assurance workflows.</p><div className="frameworks">{frameworks.map(f => <span key={f}>{f}</span>)}</div><Link href={demoHref} className="site-primary">Explore compliance →</Link></div><div className="evidence-card"><div className="evidence-row"><span>Control</span><b>Access Control</b><em>Mapped</em></div><div className="evidence-row"><span>Evidence</span><b>Identity configuration</b><em>Verified</em></div><div className="evidence-row"><span>Finding</span><b>Privileged access gap</b><em>High</em></div><div className="evidence-row"><span>Remediation</span><b>Owner + due date</b><em>Tracked</em></div><div className="evidence-row"><span>Report</span><b>Executive evidence package</b><em>Ready</em></div></div></section>

        <section className="demo-section" id="demo"><div><span className="site-eyebrow">DEMO WORLD</span><h2>See the operating model before you connect anything.</h2><p>Register a demo organization, select a limited asset scope such as a firewall, router, switch or server, then run synthetic hardening, vulnerability or standards-readiness assessments before connecting a real customer-controlled agent.</p><Link href={demoHref} className="site-primary large">Launch limited demo →</Link></div><div className="demo-list"><div><b>01</b><span>Sign in and register a demo organization</span></div><div><b>02</b><span>Select a limited asset and security module</span></div><div><b>03</b><span>Map a firewall, router, switch, server, application or API</span></div><div><b>04</b><span>Run a safe assessment and generate standards-mapped evidence</span></div></div></section>

        <section className="section pricing-section" id="pricing"><div className="section-heading"><span className="site-eyebrow">COMMERCIAL PLATFORM</span><h2>Start with the capabilities you need. Expand as your security program grows.</h2><p>CyberOS will use plans, modules, add-ons, usage and entitlements to control access across the platform.</p></div><div className="pricing-grid"><div className="price-card"><span>FOUNDATION</span><h3>Core visibility</h3><p>Organization, assets, exposure, posture and foundational reporting.</p><Link href={demoHref}>Talk to sales →</Link></div><div className="price-card featured"><span>SECURITY OPERATIONS</span><h3>Unified security</h3><p>Threat, vulnerability, assessments, compliance and operational workflows.</p><Link href={demoHref}>Explore package →</Link></div><div className="price-card"><span>ENTERPRISE</span><h3>Scale &amp; control</h3><p>Enterprise identity, private deployment, integrations and advanced controls.</p><Link href={demoHref}>Contact enterprise →</Link></div></div></section>
